...
// Added for Mobile App parity via Stripe Checkout Sessions directly from backend
export const createCheckoutSession = asyncHandler(async (req, res) => {
    try {
        const { items, successUrl, cancelUrl, customerEmail, metadata } = req.body;

        if (!items || !Array.isArray(items) || items.length === 0) {
            return res.status(400).json({ success: false, message: "Invalid checkout items" });
        }

        if (!successUrl || !cancelUrl) {
            return res.status(400).json({ success: false, message: "Success and cancel URLs are required" });
        }
        ...
        if (!session.url) {
            return res.status(500).json({ success: false, message: "Failed to generate checkout URL" });
        }
