...
const getAnalyticsOverview = asyncHandler(async (req, res) => {
    try {
        ...
        //get low stock products (stock<10 && stock>0)
...
const getProductAnalytics = asyncHandler(async (req, res) => {
    try {
        const { page = 1, limit = 20, sortBy = "totalSold", sortOrder = "desc" } = req.query
        ...
        const productAnalytics = await Order.aggregate([
            ...
            },
            {
                $addFields: {
                    ...
                    category: {
                        $arrayElemAt: ["$productDetails.category", 0]
                    },
...
const getInventoryAlerts = asyncHandler(async (req, res) => {
    try {
        ...
        }).select("name stock price image category brand createdAt");

        // Out of stock products
        const outOfStockProducts = await Product.find({ stock: 0 }).select(
            "name stock price image category brand createdAt"
        );
        ...
        }).select("name stock price image category brand updatedAt");
