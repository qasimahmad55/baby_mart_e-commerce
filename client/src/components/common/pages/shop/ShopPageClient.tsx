...
import EmptyListDesign from '../product/EmptyListDesign';
...
const ShopPageClient = ({ categories, brands }: Props) => {
    ...
    useEffect(() => {
        ...
        if (categoryFromUrl) {
            const categoryExists = categories.some((cat) => cat._id === categoryFromUrl)
            if (!categoryExists) {
                const categoryName = categories.find((cat) => cat.name.toLocaleUpperCase() === categoryFromUrl.toLocaleLowerCase())
